...
public class SwipeMenuView extends ViewGroup {
    ...
    private boolean isSwipeEnable = false;//删除功能的开关,默认开
    ...
    private boolean iosInterceptFlag = true;//IOS类型下，是否拦截事件的flag

    //20160929add 左滑右滑的开关
    private boolean isLeftSwipe = true;//   true-右滑 false-左滑
    ...
    private void init(Context context) {
        ...
        CLogger.d("mScaleTouchSlop = " + mScaleTouchSlop);
...
//                        getParent().requestDisallowInterceptTouchEvent(true);(注释 cy 2017-9-7: 不拦截事件，关闭侧滑菜单时，任然可以上下滑动)
                    }
...
/*                    if (!mScroller.isFinished()) {
                ...
                case MotionEvent.ACTION_CANCEL:
                    ...
                    //注释 cy 2017-9-7: 处理滑动时不触发item点击事件
                    float xf = Math.abs(downX - lastX);
                    float yf = Math.abs(downY - lastY);
                    if ((xf - yf) > 20) {//处理滑动时不触发item点击事件
                        getParent().requestDisallowInterceptTouchEvent(true);
                        return true;
                    }
