 ...
 * @author cy
 * @date 2018/11/26
 * 使用说明：https://github.com/Vgecanshang/CyongLibrary
 * 如果把项目的targetSdkVersion改成24以下，则不需要动态申请权限（不推荐）
 * <!-- 危险权限(需要动态申请) start -->
 * <!--PHONE-->
...
public class PermissionUtil {
    ...
    public PermissionUtil(@NonNull final FragmentActivity activity) {
        mRxPermissionsFragment = getLazySingleton(activity.getSupportFragmentManager());
    }

    public PermissionUtil(@NonNull final Fragment fragment) {
        ...
        return new Lazy<PermissionsFragment>() {
            ...
            public synchronized PermissionsFragment get() {
                if (permissionsFragment == null) {
                    permissionsFragment = getRxPermissionsFragment(fragmentManager);
                }
    ...
    private PermissionsFragment getRxPermissionsFragment(@NonNull final FragmentManager fragmentManager) {
        PermissionsFragment permissionsFragment = findRxPermissionsFragment(fragmentManager);
    ...
    private PermissionsFragment findRxPermissionsFragment(@NonNull final FragmentManager fragmentManager) {
